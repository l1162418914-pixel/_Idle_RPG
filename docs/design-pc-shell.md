...
| **克鲁赛德战记 CQ** | **接战条**：行动逻辑、自动战斗、停滚/左撤 | 消块、街景大营 |
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│ 顶栏 ~40px：金币 | 团队稳定 | 养伤锁(若有) | 当前地图名                    │
├────────────────────────────── 上区 ~70%（HSplitter 可拖）────────────────┤
│  左窗 ~32%          │  中窗 ~36%          │  右窗 ~32%（可折叠竖条）      │
│  地图/出征/log      │  编组/英雄          │  背包/出征网格              │
├══════════════════════════════ 分割条 ═════════════════════════════════════┤
│ 底栏 Run 条 ~30%（≥220px）：CombatView + 距离/返程/接战 + 稳定/价值条      │
├─────────────────────────────────────────────────────────────────────────┤
│ Dock ~48px：[出征][编组][背包][地图][后勤]  提示区  [再战][设置]           │
└─────────────────────────────────────────────────────────────────────────┘
```

### 持久规则

1. **RUNNING 时 CombatView 不得全屏盖住上区**（THB 核心：上管下打）。
