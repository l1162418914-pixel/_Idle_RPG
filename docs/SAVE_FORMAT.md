...
读档时 `squad_stability` / `team_stability` 仅作 **兼容旧档**；新档写入时二者均为占位值，**半组稳定由编队实时聚合**，见下文 §稳定度。
...
|------|------|------|
| `gold` | int | 大营金币 |
| `team_stability` | int | **已废弃语义**；兼容读档，写入占位；UI/出征起点见半组聚合 |
...
```json
{
  "active_half": "A",
  "A": {
    "active": ["elite_01", "normal_01", "…最多 4 个佣兵出战 id"],
...
- **主角不进** `A`/`B` 的 `active`/`bench`（T-02c）；读档若旧档含 `player_01`，`ensure_formation` 会剥离。
...
- **不单独存档**；由 `squad_formation` 出战 **4 槽**实时聚合（**替补 2 槽不计入总和**）
- 公式：`half_sum = Σ 出战槽 personal_stability`；`half_max = Σ 出战槽 personal_max`；空槽贡献 0
- 显示：`230/230`（`format_half_stability_text`）
- **下趟出征**时，本趟 `team_stability` 起点 = `half_sum`；`team_stability_max` = `half_max`
- 编组优先半组（`active_half`）与出征半组（`last_deploy_half`）可不同
- 备战席 / 未编入**不计入**半组总和

### 本趟团队 `team_stability`（运行时）

- **严格等于**本趟出战在编佣兵 `personal_stability` 之和（个人池聚合）
- 受击先扣个人稳，团队条随个人池同步；探索衰减等损失分摊到各员个人稳
- 某员个人条耗尽（→0）时，其余在编各扣该员上限的 **10%**（牵连，数值可调）
- **替补上阵**（压力换人 / 战间补员）时纳入总和并重算 `team_stability_max`
- 强制返程线 = `floor(team_stability_max × 30%)`
- 根存档 `team_stability` / `squad_stability`：**已废弃写入**（读档兼容）

### 个人 `personal_stability`（每名佣兵）

- 随佣兵对象保存；上限因职业/模板而异（见 `personal_stability_max` / `get_personal_stability_max()`）
- 崩溃线 = 该佣兵上限的 **30%**（非固定 30 点）
- 受击可额外扣除；低于崩溃线可触发 `is_personal_break` / 当次撤离
- 回城后在医疗室与生命一起恢复；通关地图额外扣稳定作用于**上趟出征半组**在编佣兵的个人稳定
