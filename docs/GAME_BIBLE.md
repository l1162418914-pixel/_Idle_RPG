...
| 大营观感 | **CQ 底栏动画舞台**（营火/休整/行军/接战均在屏幕下方有动画）+ **THB 上区管理**（三窗 + Dock） |
...
| 手游 Tab 大营 | 不做底部五页切换 |
| CQ 消块战斗 | 不抄三消；战斗自动打 |
| CQ 全屏街景大营横滑 | 不做；改为 **固定三窗内的营地舞台**（见 T-UI-CAMP） |
...
| 大营 UI | B1～B4 ✅；**T-UI-CAMP** CQ 营地舞台定案待开发 |
...
| **T-UI-CAMP** | CQ 营地舞台 + 建筑图标条 + BASE 底栏营火；见 [design-base-ui.md](design-base-ui.md) §T-UI-CAMP |
...
| 3 | **克鲁赛德战记 CQ** | **出征**：前后排、停滚接战、左撤、接战条槽位；**大营**：营地舞台、队伍横排、建筑图标氛围（非街景横滑） | 消块、**全屏街景大营横滑** | ⚠️ 行动+大营观感；在 CQ 上改进 | `CombatController`、`CombatView`、`RunMarchLane`；中窗 `FormationUI` / **T-UI-CAMP** |
...
| 窗口怎么分、网格在哪？ | **THB** | — |
| 大营像不像营地、队伍怎么站？ | **CQ**（舞台层） | 全屏街景滚动 |
| Dock/选图/出征怎么点？ | **THB** | CQ 街景导航 |
...
| 大营 | 整备、规划 | **底栏营火 idle 动画** + THB 上区三窗；挂机时「画面在下方」 |
...
| CQ 全屏街景大营 | **不做**；**营地舞台**（T-UI-CAMP）✅ 定案 |
| 稳定档位触发行为事件 | 血统文档标可选 |
| 返程后段降低掉物概率 | 可选 |
| 研究所 / 转生玩法 | 数据在，玩法未接 |
...
| 2026-06-07 | **CQ 大营营地舞台 + THB 交互**定案；T-UI-CAMP 任务线 |
| 2026-06-08 | **底栏 CQ 动画舞台**升为 P0（T-UI-STAGE）；中窗 CampStage 降为缩略预览 |
