extends Node
## 在编辑器 F6 运行本场景，或:
## godot --headless --path <项目根> --scene res://tools/GenerateTestSave.tscn
...
func _equip_generated(merc: Mercenary, count: int) -> void:
	...
	for i in range(count):
		...
		si += 1
		var q: int = 2 + (i % 3)
		var eq: Equipment = Equipment.generate(slot_name, q, merc.level)
		if eq != null:
...
func _fill_test_inventory() -> void:
	...
	for i in range(24):
		...
		var q: int = i % 6
		var eq: Equipment = Equipment.generate(slot_name, q, 10 + i)
		if eq != null:
